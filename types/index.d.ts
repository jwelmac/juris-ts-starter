// Adapted from https://github.com/jurisjs/juris-type-safe-template/blob/master/types/index.d.ts
...
declare module 'juris' {
  // Smart utility types that handle both sync and async variants
  export type SafeDotNotation<T> = T extends Record<string, any>
    ? { [K in keyof T & string]:
      | K
      | `${K}.${PathTo<T[K]> & string}` }[keyof T & string]
    : never;
  ...
  export type SafePathValue<T, P extends string> =
    Split<P, "."> extends [infer K, ...infer Rest]
    ? K extends keyof T
    ? Rest extends []
    ? T[K]
    : Rest extends string[]
    ? SafePathValue<T[K], Rest[number] & string>
    : never
    : never
    : never;
  ...
  export type AsyncCapable<T> = T extends (...args: any[]) => any
    ? (...args: Parameters<T>) => MaybeAsync<ReturnType<T>>
    : MaybeAsync<T>;

  // Smart function types that handle both sync and async
  export type SmartFunction<T> = (() => AsyncCapable<T>) | AsyncCapable<T>;
  export type SmartEventHandler<E = Event> = ((event: E) => void | Promise<void>);
  ...
  export type DynamicElement = JurisVDOM.BaseElementProps & {
  ...
  export interface JurisConfig<
    TState = any,
    TServices = any,
    THeadlessComponents extends Record<string, HeadlessComponentConfig<any, any, any>> = Record<string, HeadlessComponentConfig<any, any, any>>
  > {
    ...
    components?: Record<string, JurisComponentFunction<TState, TServices>>;
    headlessComponents?: THeadlessComponents;
  ...
  export type HeadlessComponentFunction<TState = any, TProps = Record<string, any>, TAPI = Record<string, any>> = (
    props: TProps,
    context: JurisContext<TState>
  ) => HeadlessComponent<TAPI>;

  export interface HeadlessComponentConfig<TState = any, TProps = Record<string, any>, TAPI = Record<string, any>> {
    fn: HeadlessComponentFunction<TState, TProps, TAPI>;
  ...
  export namespace JurisVDOM {
    ...
    export type TextAreaElement = BaseElementProps & {
      value?: string | (() => string);
      defaultValue?: string;
    ...
    export type SelectElement = BaseElementProps & {
    ...
    // Link and navigation
    export type LinkElement = BaseElementProps & {
      href: string | (() => string);
      text?: string | (() => string);
      children?: Element[] | (() => Element[]);
      target?: '_blank' | '_self' | '_parent' | '_top' | string;
  ...
  type RegisteredComponentElements = keyof Juris.RegisteredComponents extends never
    ...
    : {
      [K in keyof Juris.RegisteredComponents]: {
        [P in K]: JurisVDOM.ComponentElement & Juris.RegisteredComponents[K]
      }
    }[keyof Juris.RegisteredComponents];

  export type ValidateComponent<T> = JurisVDOM.ValidateComponentElement<T>;
  ...
  type JurisContextCore<TServices = any, THeadlessComponents = any> = ComponentState<any> & TServices & {
    services?: TServices;
    ...
    components?: {
      register: (name: string, component: JurisComponentFunction<TState, TServices, THeadlessComponents>) => void;
      registerHeadless: (name: string, component: any, options?: any) => void;
      get: (name: string) => JurisComponentFunction<TState, TServices, THeadlessComponents> | undefined;
      getHeadless: <K extends keyof THeadlessComponents>(name: K) => ReturnType<THeadlessComponents[K]['fn']>;
      initHeadless: (name: string, props?: any) => any;
      reinitHeadless: (name: string, props?: any) => any;
      getHeadlessAPI: <K extends keyof THeadlessComponents>(name: K) => ReturnType<THeadlessComponents[K]['fn']>['api'];
  ...
  export type JurisContext<TState = any, TServices = any, THeadlessComponents = any> = JurisContextCore<TServices, THeadlessComponents> & ComponentState<TState> & {
  ...
  // Headless component
  export interface HeadlessComponent<API = Record<string, any>> {
    api?: API;
    hooks?: {
      onRegister?: () => void;
      onUnregister?: () => void;
  ...
  export interface JurisInstance<TState = {}, TServices = {}, THeadlessComponents = {}> extends ComponentState<TState> {
    ...
    registerComponent: (name: string, component: JurisComponentFunction<TState, TServices>) => void;
    registerHeadlessComponent: (name: string, component: HeadlessComponentFunction<TState, TServices>, options?: any) => void;
    getComponent: (name: string) => JurisComponentFunction<TState, TServices> | undefined;
    getHeadlessComponent: <K extends keyof THeadlessComponents>(name: K) => ReturnType<THeadlessComponents[K]['fn']>;
    ...
    cleanup: () => void;
    destroy: () => void;
    createContext: (element?: HTMLElement) => JurisContext<TState, TServices>;
    createHeadlessContext: (element?: HTMLElement) => JurisContext<TState, TServices>;
  ...
  export interface JurisConstructor<TState = any, TServices = any, THeadlessComponents extends Record<string, HeadlessComponentConfig<any, any, any>> = Record<string, HeadlessComponentConfig<any, any, any>>> {
    new <TState, TServices, THeadlessComponents>(config?: JurisConfig<TState, TServices, THeadlessComponents>): JurisInstance<TState, TServices, THeadlessComponents>;
  }
  ...
  declare global {
    namespace Juris {
      ...
      type ComponentDefinition = keyof RegisteredComponents extends never
        ? { [K in string]: JurisVDOM.ComponentElement }
        : { [K in keyof RegisteredComponents]: JurisVDOM.ComponentElement & RegisteredComponents[K] };
    }
  }
